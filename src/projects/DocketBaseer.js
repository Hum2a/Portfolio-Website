...
const docketProject = getProjectById("docket-baseer");
const surfaceCategories = getProjectCategories(docketProject);

const terminalLines = [
  "boot docket — Baseer's job pipeline",
...
const projectInfo = `const project = {
  ...
  description:
    "Kanban + list + docs + stats for Baseer's search, with Resend alerts when roles are logged or statuses move.",
  stack: [
    "React 19",
    "Vite",
    "Hono",
...
const DocketBaseer = () => {
  return (
    <ProjectLayout
      ...
      terminalSpeed={32}
      logo={`${process.env.PUBLIC_URL}/logos/DocketBaseer.png`}
    ...
    >
      <section className="project-section">
        ...
        <p className="section-description">
          A Docket instance built for <strong>Baseer</strong>: applications move through wishlist →
          ...
          SPA (Turborepo), with Neon Postgres (RLS-ready), R2 for private files, and Resend emails
          when roles are logged or status changes. Single-owner via fixed <code>OWNER_ID</code> —
          no accounts.
        </p>
        <p className="section-description">
          The live site holds real application data, so this page links out rather than embedding a
          public iframe. Related: Humza&apos;s own Docket at <Link to="/docket">/docket</Link>.
        </p>
      </section>
